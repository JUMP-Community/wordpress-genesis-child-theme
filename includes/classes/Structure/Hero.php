...
class Hero {
	...
	public function setup() {
		add_action( 'genesis_meta', [ $this, 'hero_setup' ] );
	}
	...
	public function hero_setup() {
		...
		\add_filter( 'genesis_attr_archive-title', [ $this, 'hero_archive_title_attr' ] );
		\add_filter( 'genesis_attr_entry', [ $this, 'hero_entry_attr' ] );
		\add_filter( 'body_class', [ $this, 'hero_body_class' ] );
		...
		\add_action( 'genesis_archive_title_descriptions', [ $this, 'do_archive_headings_intro_text' ], 12, 3 );
		\add_action( 'genesis_hero_section', [ $this, 'hero_title' ], 10 );
		\add_action( 'genesis_hero_section', [ $this, 'hero_excerpt' ], 20 );
		\add_action( 'be_title_toggle_remove', [ $this, 'hero_title_toggle' ] );
		\add_action( 'genesis_before_content', [ $this, 'hero_remove_404_title' ] );
		\add_action( 'genesis_before_content_sidebar_wrap', [ $this, 'hero_display' ] );
	 ...
	 * @return array
	 */
	public function hero_body_class( array $classes ) : array {
	 ...
	 * @return void
	 */
	public function hero_remove_404_title() {
		if ( \is_404() ) {
	...
	public function hero_title() {
		if ( \is_home() && 'posts' === \get_option( 'show_on_front' ) ) {
			$title = \apply_filters( 'genesis_latest_posts_title', esc_html__( 'Latest Posts', 'jump' ) );

		} elseif ( \is_404() ) {
			$title = \apply_filters( 'genesis_404_entry_title', esc_html__( 'Not found, error 404', 'jump' ) );

		} elseif ( \is_search() ) {
			$title = \apply_filters( 'genesis_search_title_text', esc_html__( 'Search results for: ', 'jump' ) . \get_search_query() );
	 ...
	 * @return void
	 */
	public function hero_excerpt() {
		$excerpt = '';
		$id      = '';

		if ( \is_home() && 'posts' === \get_option( 'show_on_front' ) ) {
			$excerpt = \apply_filters( 'genesis_latest_posts_subtitle', esc_html__( 'Showing the latest posts', 'jump' ) );
	 ...
	 * @return void
	 */
	public function do_archive_headings_intro_text( string $heading = '', string $intro_text = '', string $context = '' ) {
	 ...
	 * @return array
	 */
	public function hero_archive_title_attr( array $atts ) : array {
	 ...
	 * @return array
	 */
	public function hero_entry_attr( array $atts ) : array {
		if ( \is_singular() ) {
